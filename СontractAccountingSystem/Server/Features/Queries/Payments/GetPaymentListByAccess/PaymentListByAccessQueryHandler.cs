...
{
    public class PaymentListByAccessQueryHandler : IRequestHandler<PaymentListByAccessQuery, List<PaymentTermModel>>
    {
        ...
        public PaymentListByAccessQueryHandler(Repository repository, IMapper mapper, DocumentService documentService, UserRepository userRepository)
        {
        ...
        public async Task<List<PaymentTermModel>> Handle(PaymentListByAccessQuery request, CancellationToken cancellationToken)
        {
            ...
            {
                if (!docList.Any(x => x.Id == entity.DocumentId))
                    continue;
